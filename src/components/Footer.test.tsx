...
describe('Footer Component', () => {
  it('renders footer with correct structure', () => {
    ...
    expect(
      screen.getByText(/professional web design and development services/i)
    ).toBeInTheDocument();
  });
  ...
  it('renders all service links correctly', () => {
    ...
    // Check service links
    expect(
      screen.getByRole('link', { name: /ui\/ux design/i })
    ).toBeInTheDocument();
    expect(
      screen.getByRole('link', { name: /front-end development/i })
    ).toBeInTheDocument();
    expect(
      screen.getByRole('link', { name: /full-stack development/i })
    ).toBeInTheDocument();
    expect(
      screen.getByRole('link', { name: /web consulting/i })
    ).toBeInTheDocument();
  });
  ...
  it('renders social media links with proper accessibility', () => {
    ...
    expect(
      screen.getByRole('link', { name: /follow us on linkedin/i })
    ).toBeInTheDocument();
    expect(
      screen.getByRole('link', { name: /follow us on github/i })
    ).toBeInTheDocument();
  });
  ...
  it('renders copyright with current year', () => {
    ...
    expect(
      screen.getByText(
        new RegExp(`© ${currentYear} MPDEE. All rights reserved.`)
      )
    ).toBeInTheDocument();
  });
  ...
  it('has proper link attributes for navigation', () => {
    ...
    servicesLinks.forEach(link => {
      if (
        link.textContent?.includes('Services') &&
        !link.textContent?.includes('UI/UX')
      ) {
