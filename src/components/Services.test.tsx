...
jest.mock('framer-motion', () => ({
  motion: {
    ...
    section: ({ children, initial, animate, transition, whileInView, viewport, ...props }: any) => <section {...props}>{children}</section>,
  },
...
describe('Services Component', () => {
  ...
  it('renders services section with correct structure', () => {
    ...
    // Check description
    expect(screen.getByText(/we deliver comprehensive web solutions/i)).toBeInTheDocument();
    
    // Check main CTA button
    expect(screen.getByRole('button', { name: /discuss your project/i })).toBeInTheDocument();
  });
  ...
  it('renders all three service cards', () => {
    ...
    expect(screen.getByRole('heading', { level: 3, name: /ui\/ux design/i })).toBeInTheDocument();
    expect(screen.getByRole('heading', { level: 3, name: /front-end development/i })).toBeInTheDocument();
    expect(screen.getByRole('heading', { level: 3, name: /full-stack development/i })).toBeInTheDocument();
    
    // Check service descriptions
    expect(screen.getByText(/creating intuitive and visually appealing/i)).toBeInTheDocument();
    expect(screen.getByText(/building responsive, interactive web applications/i)).toBeInTheDocument();
    expect(screen.getByText(/end-to-end web application development/i)).toBeInTheDocument();
  ...
  it('opens modal when Learn More button is clicked', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('UI/UX Design', { selector: '#modal-title' })).toBeInTheDocument();
    });
    ...
    expect(screen.getByText(/user research and persona development/i)).toBeInTheDocument();
  ...
  it('handles main Discuss Your Project button', () => {
    ...
    const discussButton = screen.getByRole('button', { name: /discuss your project/i });
  ...
  it('has proper accessibility attributes', () => {
    ...
    const discussButton = screen.getByRole('button', { name: /discuss your project with us/i });
